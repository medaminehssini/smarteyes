...
import {objectDetection} from '../mlkit';

import TransparentHeader from '../components/TransparentHeader';
import Tts from 'react-native-tts';

export const ObjectDetectionScreen = ({navigation}: any) => {
  ...
  const onTakePhoto = async () => {
    try {
      ...
      try {
        const response = await objectDetection(uri);
        console.log(response);

        if (response?.nbr > 0) {
          Tts.speak('The picture is captured successfully');
          navigation.navigate('ResultObjectDetection', {
            url: uri,
            response: response,
          });
        } else {
